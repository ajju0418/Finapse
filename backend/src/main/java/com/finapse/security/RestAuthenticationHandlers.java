...
/**
 * Renders authentication/authorization failures as the same JSON envelope the
 * rest of the API uses, so the frontend has one error shape to handle.
 */
@Component
...
public class RestAuthenticationHandlers implements AuthenticationEntryPoint, AccessDeniedHandler {
    ...
    private void write(HttpServletResponse response, HttpStatus status, String code, String message)
        ...
        objectMapper.writeValue(response.getWriter(),
                ErrorResponse.of(status.value(), code, message));
    }
}
